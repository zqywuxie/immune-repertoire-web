...
file_dir = "/data/scAnalyis/260416/newanal/DB_pathology/specify_ratio"
...
def plotboxs_mat_mutiple(
    ...
    edge_palette_dict = make_edge_palette(class_col)
    box_edge_colors = [edge_palette_dict[group] for group in class_dict[class_col]]

    for patch, edge_color in zip(ax_box.patches, box_edge_colors):
        patch.set_edgecolor(edge_color)
        patch.set_linewidth(0.9)

    if ax_box.patches:
        lines_per_box = max(1, len(ax_box.lines) // len(ax_box.patches))
        for i, edge_color in enumerate(box_edge_colors):
            for line in ax_box.lines[i * lines_per_box : (i + 1) * lines_per_box]:
    ...
    sns.stripplot(
        ...
        x=class_col,
        data=concat_df,
        palette=make_edge_palette(class_col),
        jitter=0.2,
        alpha=0.72,
        size=3.0,
        linewidth=0,
    ...
    ax_box.tick_params(labelsize=tick_font_size, length=2.5, width=0.6)

...
class TestHistoryFunctionality:
    """Task 13.7: Test 历史记录功能 (Requirements: 10.1, 10.2, 10.3)"""
    
    @pytest.fixture
    def history_test_file(self, client):
        """Upload a test file for history tests"""
        csv_content = b"""Sample,Data1,Data2
S1,10,20
S2,30,40"""
        
        response = client.post(
            '/api/files/upload',
            data={'file': (BytesIO(csv_content), 'history_test.csv')},
            content_type='multipart/form-data'
        )
        if response.status_code != 201:
            pytest.skip(f"File upload not available (status {response.status_code})")
    ...
    def test_history_records_scheme_info(self, client, history_test_file):
        """Test that history records scheme information (Requirements 10.1, 10.2)"""
        file_id = history_test_file['id']
            ...
            if response.status_code in [200, 201]:
                ...
                if analysis_id:
                    ...
                    if 'history' in history_data:
                        ...
                        analysis_history = next((h for h in history_items if h.get('id') == analysis_id), None)
                        
                        if analysis_history:
                            # Verify scheme info is recorded
                            assert 'mode' in analysis_history or 'type' in analysis_history
    
    def test_history_records_custom_fields(self, client, history_test_file):
        """Test that history records custom field information (Requirements 10.1, 10.2)"""
        file_id = history_test_file['id']
        ...
        if response.status_code in [200, 201]:
            ...
            if analysis_id:
                ...
                # Verify custom fields are recorded
                if 'history' in history_data:
                    history_items = history_data['history']
                    analysis_history = next((h for h in history_items if h.get('id') == analysis_id), None)
                    
                    if analysis_history:
                        assert 'mode' in analysis_history or 'selected_fields' in analysis_history
    
    def test_rerun_analysis_from_history(self, client, history_test_file):
        """Test re-running analysis from history (Requirement 10.3)"""
        file_id = history_test_file['id']
        ...
        if response.status_code in [200, 201]:
            ...
            if analysis_id:
                # Try to rerun the analysis
                rerun_response = client.post(f'/api/analysis/{analysis_id}/rerun')
                
                # Should succeed or return error if rerun not supported
                assert rerun_response.status_code in [200, 201, 400, 404]
    
    def test_multiple_analyses_in_history(self, client, history_test_file):
        """Test that multiple analyses are recorded in history"""
        file_id = history_test_file['id']
        ...
        for i in range(3):
            analysis_request = {
                'file_id': file_id,
                'mode': 'custom',
                'selected_fields': ['Sample', f'Data{(i % 2) + 1}'],
                'parameters': {}
            }

...
const AutoHeatmap = {
    ...
    async browseRemote(path = null, skipLoading = false) {
        ...
        if (!skipLoading) this.showLoading('Loading remote folders...');
    ...
    async pollSyncTaskStatus(taskId) {
        try {
            ...
            if (loadingText) {
                loadingText.textContent = data.detail || `Sync progress: ${Math.round(Number(data.progress || 0))}%`;
            }
    ...
    async scanLocalFolder(basePath, loadingText = 'Scanning folder...') {
        ...
        try {
            ...
            const data = await response.json();
            if (!data.success) throw new Error(data.message || 'Failed to scan folder');

            this.basePath = basePath;
            this.scanResult = data;
            this.samples = data.samples.map(s => ({ ...s, selected: true }));
            this.setScanSummary(data.summary || 'Scan completed', data.samples.length === 0 ? 'warning' : 'success');
    ...
    async scanFolder() {
        ...
        try {
            await this.scanLocalFolder(basePath, 'Scanning folder...');
        } catch (error) {
            this.showError(error.message);
        }
    },

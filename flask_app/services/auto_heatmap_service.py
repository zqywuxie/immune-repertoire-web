...
class FolderScanResult:
    ...
    all_file_types: List[str] = field(
        default_factory=list
    )  # Unique file patterns found
    all_chains: List[str] = field(
        default_factory=list
    )  # Detected chain types (e.g., TRA, TRB, IGH)
...
class FieldMapping:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"cdr3_column": self.cdr3_column, "copy_column": self.copy_column}
...
class AutoHeatmapService:
    ...
    # Derived outputs should not be treated as raw repertoire inputs during folder scan.
    AUXILIARY_DATA_MARKERS = [
        ".vj_freq.",
        ".vj_frequency.",
    ]
    ...
    CHAIN_TYPES = [
        ...
        "TRB",
        "TRG",
        "TRD",  # T cell receptor chains
        "IGH",
        "IGK",
        "IGL",  # B cell receptor chains
        "TCRA",
        "TCRB",
        "TCRG",
        "TCRD",  # Alternative naming
    ]
    ...
    def scan_base_folder(self, base_path: str) -> FolderScanResult:
        ...
        if not base_path:
            raise ValidationError(
                message="请输入分析文件夹路径", details={"path": base_path}
            )
        ...
        if not path.exists():
            raise ValidationError(
                message=f"指定路径不存在: {base_path}", details={"path": base_path}
            )

        if not path.is_dir():
            raise ValidationError(
                message=f"指定路径不是目录: {base_path}", details={"path": base_path}
            )
        ...
        try:
            ...
            for root, dirs, files in os.walk(base_path):
                ...
                if data_files_in_dir:
                    ...
                    chain_files = [
                        f
                        for f in data_files_in_dir
                        if self._extract_chain_from_filename(f.filename)
                    ]
                    ...
                    if chain_files and len(chain_files) == len(data_files_in_dir):
                        ...
                        for file_info in data_files_in_dir:
                            extracted_name = self._extract_sample_name_from_chain_file(
                                file_info.filename
                            )
                            if extracted_name:
                                if extracted_name not in sample_groups:
                                    sample_groups[extracted_name] = []
                                sample_groups[extracted_name].append(file_info)
                        ...
                        if sample_groups:
                            ...
                            for sample_name, files in sample_groups.items():
                                if sample_name in sample_dict:
                                    # 合并到已存在的样本
                                    sample_dict[sample_name].data_files.extend(files)
                                else:
            ...
            has_chain_suffix = (
                total_files > 0 and (chain_suffix_count / total_files) > 0.5
            )
    ...
    def _find_data_files_recursive(
        ...
        try:
            ...
            logger.debug(
                f"Scanning directory: {directory}, found {len(entries)} entries"
            )
            ...
            for entry in entries:
                ...
                if os.path.isfile(entry_path):
                    ...
                    sub_files = self._find_data_files_recursive(
                        entry_path, max_depth - 1
                    )
    ...
    def _is_data_file(self, filename: str) -> bool:
        ...
        if any(marker in lower_name for marker in self.AUXILIARY_DATA_MARKERS):
            return False
    ...
    def _get_file_info(self, filename: str, filepath: str) -> DataFileInfo:
        ...
        file_info = DataFileInfo(filename=filename, filepath=filepath)
    ...
    def get_file_columns(self, filepath: str) -> Dict[str, Any]:
        ...
        if not os.path.exists(filepath):
            raise ValidationError(
                message=f"文件不存在: {filepath}", details={"filepath": filepath}
            )
        ...
        try:
            ...
            suggested_cdr3 = self._find_matching_column(
                columns, self.CDR3_COLUMN_PATTERNS
            )
            suggested_copy = self._find_matching_column(
                columns, self.COPY_COLUMN_PATTERNS
            )
    ...
    def load_sample_data(
        ...
        for sample in samples:
            ...
            if not matching_file:
                logger.warning(
                    f"No matching file found for sample {sample.display_name}"
                )
                continue
            ...
            try:
                ...
                if cdr3_col not in df.columns:
                    logger.warning(
                        f"CDR3 column '{cdr3_col}' not found in {matching_file.filepath}"
                    )
                    continue

                if copy_col not in df.columns:
                    logger.warning(
                        f"Copy column '{copy_col}' not found in {matching_file.filepath}"
                    )
                    continue
                ...
                logger.error(
                    f"Error loading data for sample {sample.display_name}: {e}"
                )
                continue
    ...
    def load_sample_data_for_single_chain(
        self, samples: List[SampleFolderInfo], chain: str, field_mapping: FieldMapping
    ) -> Dict[str, pd.DataFrame]:
        """
        为单条链加载样本数据。
                        ...
                        normalized_df = pd.DataFrame(
                            {
                                "cdr3": df[cdr3_col],
                                "copy": pd.to_numeric(
                                    df[copy_col], errors="coerce"
                                ).fillna(0),
                            }
                        )
                        sample_data[sample.display_name] = normalized_df
                        logger.info(
                            f"Loaded chain {chain} for sample {sample.display_name}, {len(normalized_df)} CDR3s"
                        )
                    else:
                        logger.warning(
                            f"Required columns not found in {matching_file.filepath}"
                        )

                except Exception as e:
                    logger.error(
                        f"Error loading chain {chain} for sample {sample.display_name}: {e}"
                    )
                    continue
    ...
    def load_sample_data_by_chains(
        ...
        for chain in selected_chains:
            sample_data = self.load_sample_data_for_single_chain(
                samples, chain, field_mapping
            )
            if sample_data and len(sample_data) >= 2:
                ...
                logger.warning(
                    f"Chain {chain}: insufficient samples ({len(sample_data) if sample_data else 0})"
                )
    ...
    def calculate_all_metrics(
        ...
        return {
            ...
            "cdr3_sharing": self.calculate_similarity_matrix(
                sample_data, "cdr3_sharing"
            ),
            "expression_sharing": self.calculate_similarity_matrix(
                sample_data, "expression_sharing"
            ),
            "morisita_horn": self.calculate_similarity_matrix(
                sample_data, "morisita_horn"
            ),
            "sorensen": self.calculate_similarity_matrix(sample_data, "sorensen"),
        }

    def calculate_similarity_matrix(
        self, sample_data: Dict[str, pd.DataFrame], metric: str = "r2_inner"
    ) -> pd.DataFrame:
        """
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                if metric == "r2_inner":
                    ...
                    sim = self._calculate_morisita_horn(
                        abundance[name_i], abundance[name_j]
                    )
                    matrix[i, j] = sim
                    matrix[j, i] = sim
    ...
    def _calculate_r2_inner(
        self, abundance_a: Dict[str, float], abundance_b: Dict[str, float]
    ) -> float:
        """
    ...
    def calculate_group_averages(
        self, similarity_matrix: pd.DataFrame, groups: List[SampleGroup]
    ) -> pd.DataFrame:
        """
        ...
        for group in groups:
            valid_samples = [
                s for s in group.sample_names if s in similarity_matrix.index
            ]
            if valid_samples:
                valid_groups.append(
                    SampleGroup(
                        name=group.name, sample_names=valid_samples, color=group.color
                    )
                )
    ...
    def _calculate_within_group_average(
        self, matrix: pd.DataFrame, samples: List[str]
    ) -> float:
    ...
    def _calculate_between_group_average(
        self, matrix: pd.DataFrame, samples_a: List[str], samples_b: List[str]
    ) -> float:

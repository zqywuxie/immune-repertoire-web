...
vi.mock("../features/scripthub/stages/Stage4Execution",()=>({Stage4Execution:({modules,moduleConfigs,onJobsCreated,onComplete}:any)=><><pre data-testid="execution">{JSON.stringify({modules,moduleConfigs})}</pre><button onClick={()=>onJobsCreated(["job-1"])}>提交测试任务</button><button onClick={()=>onComplete({"job-1":{status:"completed"}})}>完成测试任务</button></>}));
vi.mock("../features/scripthub/stages/Stage6History",()=>({Stage6History:({initialJobId}:any)=><p>历史任务：{initialJobId}</p>}));
vi.mock("../features/scripthub/stages/Stage5Results",()=>({Stage5Results:()=> <p>同页分析结果</p>}));
...
async function inspect(){fireEvent.click(screen.getByRole("button",{name:"检查所选数据"}));await waitFor(()=>expect(screen.getByRole("button",{name:"下一步"})).toBeEnabled());}
describe("独立分析工具",()=>{
 ...
 it("图表独立页只能提交自己的模式，并保留可重开的任务地址",async()=>{
  ...
  fireEvent.click(screen.getByText("完成测试任务"));expect(screen.getByText("同页分析结果")).toBeInTheDocument();expect(screen.queryByRole("button",{name:"查看结果"})).not.toBeInTheDocument();
 });
 it("转录组专页不接受只有 PEP 的数据",async()=>{
  wizard("expression");fireEvent.click(screen.getByText("选择 克隆序列表 数据"));fireEvent.click(screen.getByText("检查所选数据"));

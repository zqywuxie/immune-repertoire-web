...
export function InfiltrationConfig({projectId,sourceContext,value,onChange}:ModuleFormProps) {
  ...
  useEffect(()=>{
    ...
    onChange({...value,cell_columns:[],infiltration_checked:false});
    inspectScriptHubModule<Inspection>("immune-infiltration",source).then(result=>{
      if(current!==version.current)return;
      setInspection(result);onChange({...value,cell_columns:result.cell_columns,infiltration_checked:false});
  ...
  async function verify(){
    ...
    try{
      const result=await inspectScriptHubModule<Inspection>("immune-infiltration",{...source,group_field:value.group_field,cell_columns:cells});
      if(current!==version.current)return;
  ...
  return <ModuleShell title="免疫浸润组成与组间比较" detail="分组来自样本指标表；按完整样本编号匹配，不从名称推断分组。" sourceContext={sourceContext}>
    <Section title="选择分析范围">
      ...
      <button type="button" className="btn btn-primary" disabled={busy||!value.group_field||!cells.length} onClick={verify}>{busy?"正在检查…":"核对分析范围"}</button>

...
export function ContinueAnalysis({ result }: { result: JobResultsResponse }) {
  ...
  const enabled = result.status === "completed" && [job.module, job.job_type].includes("pep-analysis") && !!project && !!dataset && !!task;
  ...
  }, [enabled, project, dataset, task, revision]);
  ...
  return <section aria-label="继续分析" style={{ display: "grid", gap: "var(--spacing-sm)" }}>
    ...
    {candidates.map(candidate => {
      const targets = candidate.cache_type === "vj_usage" ? [["vj-difference", "V/J 使用差异"], ["umapin", "UMAP 特征降维"], ["ml", "机器学习"]]
